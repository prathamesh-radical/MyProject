...
export default function TableFilterHeading({ isAllChecked, toggleAll, handleSort, sortOrder }) {
    const rotationAnim = {
        userName: useRef(new Animated.Value(
            sortOrder.column === "userName" && sortOrder.order === "ascending" ? 0 : 1
        )).current,
        eventDate: useRef(new Animated.Value(
            sortOrder.column === "eventDate" && sortOrder.order === "ascending" ? 0 : 1)
        ).current,
    };
    ...
    return (
        ...
        >
            ...
            <DataTable.Title style={{ flex: 2, position: "relative", right: 15, top: 3 }}>
                <TouchableOpacity
            ...
            <DataTable.Title style={{ flex: 3, position: "relative", left: 15, top: 3 }}>
                <TouchableOpacity
                    style={{ flexDirection: "row", gap: 5, alignItems: "center" }}
                    onPress={() => handleColumnSort("eventDate")}
                >
                    <Text style={{ fontWeight: "bold", color: "#000" }}>Event Date</Text>
                    <Animated.View
                        style={{
                            transform: [
                                {
                                    rotate: rotationAnim.eventDate.interpolate({
                                        inputRange: [0, 1],
                                        outputRange: ["0deg", "180deg"],
                                    }),
                                },
